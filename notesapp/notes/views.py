...
from .forms import NoteForm, ContactForm, SearchForm, CommentForm
...
def edit_note(request, pk):
    ...
    if request.method == 'GET':
        ...
        if form.is_valid():
            form.save()
            success(request,"Your note was updated! Great work!")
            return redirect(to='notes_list', pk=pk )
...
def delete_note(request, pk):
    ...
    if request.method == 'POST':
        note.delete()
        success(request, "Your note was deleted.")
        return redirect(to='notes_list')
